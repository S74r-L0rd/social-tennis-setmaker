...
/**
 * Test 13:
 * Verifies that planned round limits are respected by the scheduler.
 *
 * Scenario:
 * - 5 players are active
 * - 4 players planned to play 1 round and have already played it
 * - 1 player planned to play 2 rounds and has only played 1
 *
 * Expected:
 * - A second round cannot be generated by pulling quota-reached players back in
 */
(function testPlannedRoundsLimitPreventsForcedExtraRound() {
  const players = [
    { id: 1, rating: 6, sitOutCount: 0, plannedRounds: 1, roundsPlayed: 1 },
    { id: 2, rating: 6, sitOutCount: 0, plannedRounds: 1, roundsPlayed: 1 },
    { id: 3, rating: 6, sitOutCount: 0, plannedRounds: 1, roundsPlayed: 1 },
    { id: 4, rating: 6, sitOutCount: 0, plannedRounds: 1, roundsPlayed: 1 },
    { id: 5, rating: 6, sitOutCount: 0, plannedRounds: 2, roundsPlayed: 1 },
  ];

  assert.strictEqual(isEligibleForRound(players[0]), false);
  assert.strictEqual(isEligibleForRound(players[4]), true);

  assert.throws(
    () => generateRound(players, ["Court 1"], emptyHistory()),
    /at least 4 eligible players/
  );

  console.log("Test 13 passed: planned round limits prevent forced extra rounds");
})();

/**
 * Test 14:
 * Verifies that players who cannot fit into a complete doubles match
 * are recorded as sit-outs instead of being silently dropped.
 *
 * Scenario:
 * - 5 eligible players
 * - 2 courts are available, but only 1 full match can be formed
 *
 * Expected:
 * - 1 match should be generated
 * - 1 player should be listed as sitting out
 */
(function testIncompleteDoublesGroupBecomesSitOut() {
  const players = [
    { id: 1, rating: 6, sitOutCount: 0 },
    { id: 2, rating: 6, sitOutCount: 0 },
    { id: 3, rating: 6, sitOutCount: 0 },
  ...
  const result = generateRound(players, ["Court 1", "Court 2"], emptyHistory());

  assert.strictEqual(result.matches.length, 1);
  assert.strictEqual(result.sitOuts.length, 1);

  console.log("Test 14 passed: incomplete doubles group is tracked as sit-out");
})();
